(ns pi.operation
  "OperationActor -- one PI (Payment Institution) operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (PaymentOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the PIGovernor (:govern) and the
  ...
  One graph run = one PI operation (intake -> advise -> govern -> decide
  ...
  `{:approval {:status :approved}}` (or :rejected). `:actuation/execute-
  payment`/`:actuation/remit-payout` ALWAYS reach this node when the
  governor is clean -- see `pi.phase`."
...
(defn- commit-fact
  ...
  (nil for an auto-commit, e.g. phase 3's :account/intake) -- without
  this, the ledger cannot answer 'approved by whom'."
...
(defn build
  ...
  `pi.store/Store`).
  opts:
    :advisor      -- a `pi.piadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (piadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PaymentOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (piadvisor/-advise advisor store request)]
            {:proposal p :audit [(piadvisor/trace request p)]})))

      ;; PIGovernor -- independent censor (separate system than the LLM).
      (g/add-node :govern
